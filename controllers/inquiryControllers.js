...
// SET INQUIRY AS SOLVED
const updateAsSolved = async(req, res) => {
    try {
        const filter = {inquiryId: req.params.inquiryId};
        ...
        const inquiry = await Inquiry.findOneAndUpdate(filter, updatedInquiry, {new:true});

        res.status(200).json({message:"Inquiry is updated as solved", inquiry});
    } catch (error) {
        res.status(500).json({message:"Error in updating inquiry as solved", error});
