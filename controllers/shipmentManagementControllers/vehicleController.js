...
function calculateArrivalTimes(route, deliveryType, timeMatrix) {
    ...
    // For logging purposes
    console.log(`Arrival times calculated for route:`);
    arrivalTimes.forEach(at => console.log(`  ${at.center}: ${at.time}h`));
    console.log(`Shipment finish time: ${shipmentFinishTime}h`);
...
async function assignVehicle(shipmentId, shipmentType) {
    try {
        ...
        if (vehicleBranchId.equals(sourceCenterId)) {
            ...
            // Create a reverse shipment to bring the vehicle to the source center
            await createReverseShipment(vehicle, vehicleBranchId, sourceCenterId, shipmentType, matrices);
        }
        ...
        return {
            ...
            message: vehicleBranchId.equals(sourceCenterId) ?
                `Vehicle ${vehicle.vehicleId} assigned to shipment` :
                `Vehicle ${vehicle.vehicleId} from ${vehicleLocation} assigned and will be dispatched to ${sourceLocation}`,
            vehicle: vehicle
        };
    } catch (error) {
        console.error("Error assigning vehicle:", error);
 ...
 */
async function createReverseShipment(vehicle, fromCenterId, toCenterId, shipmentType, matrices) {
    try {
        ...
        console.log(`Creating reverse shipment for vehicle ${vehicle.vehicleId} from ${fromLocation} (${fromCenterId}) to ${toLocation} (${toCenterId})`);
        ...
        if (pendingParcels.length === 0) {
            console.log(`No parcels found for reverse shipment from ${fromLocation} to ${toLocation}`);
            return false;
        }
        ...
        const reverseShipmentId = `${shipmentType === "Express" ? "EX" : "ST"}-S${shipmentCount.toString().padStart(3, '0')}-${fromLocation}`;

        // Calculate the route details - route is array of ObjectIds
