...
//   // If Base64 image is sent in the request
...
//         httpOnly:true,
//         secure:process.env.NODE_ENV==='production',//omly secure in production
...
//     const {email,password,passwordconfirm,fName,lName,address,nic,contact,province,district,zone}=req.body;
...
//         return next(new AppError('Otp has expired.Please requiest a new OTP',400));
//     }
...
//         user.otpExpires=undefined;
//         await user.save({validateBeforeSave:false})
//         return next(new AppError("there is a error sending the email.!",500))
//      }
// });
...
//     //compare password savepassword and this
...
//         await sendEmail({
//             email:user.email,
//           subject:"Your Password Reset OTP(valid for 5 minitues)",
...
//            res.status(200).json({
//             status:"success",
//             message:"Password Reset Otp sent Your email",   
//            });
//     } catch (error) {
...
exports.resendOTP = catchAscync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email });
  if (!user) return next(new AppError("User not found", 404));
  if (user.isVerify) return next(new AppError("This account is already verified!", 400));
